...
M2 里故意让 `products.json` 的 id=27 缺 `specs`、id=28 缺 `review_summary`。如果工具用下标 `p["review_summary"]` 访问，遇到缺字段会直接 `KeyError` 崩溃，整个 agent 挂掉。所以工具层要用 `.get("review_summary", "暂无评价")` 防御。
...
- **脏数据防御**：数据扩到 30 条，故意含缺字段的脏数据，工具层用 `.get()` 防御（见知识点 7）。
- **评估 v2**：任务集扩到 30 条分 5 类，指标拆成「成功率」+「质量分」（见知识点 8）。

> 关于"真实数据源"：M2 用"带脏数据的 mock"代替了爬京东/淘宝。反爬+合规风险大、无经验容易卡死；"处理不规整数据"的能力已通过脏数据练到。M2 后段再决定是否接公开商品 API。
